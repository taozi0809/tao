...
<script>
    ...
    export default {
        data(){
            return{
                account:getLocalStore('tokenholdem_account')!=null?getLocalStore('tokenholdem_account'):'',
                password:getLocalStore('tokenholdem_password')!=null?aesDe(getLocalStore('tokenholdem_password')):'',
                check:getLocalStore('tokenholdem_password')!=null?true:false,
            }
        },
        methods:{
            ...mapMutations(['TOKEN']),
            loginClick(){
                if(!!this.account&&!!this.password){
                    ...
                    login(data).then(res=>{
                        console.log('登录:',res)
                        if(res.errcode==2000){
                            setLocalStore('tokenholdem_account',this.account)
                            if(this.check){setLocalStore('tokenholdem_password',aesEn(this.password))} else{
                                removeLocalStore('tokenholdem_password')
                            }
