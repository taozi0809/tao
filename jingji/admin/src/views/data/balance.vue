    ...
    <div class="match">
        ...
        <div class="list">
            <el-table :data="list" style="width: 100%;margin-bottom: 10px;" border height="700" v-loading="loading"
                ...
                <el-table-column prop="CouponUsePlayersCount" label="拥有点券余额人数"></el-table-column>
            </el-table>

            <el-pagination
                    @current-change="getAnalysisPlayerCoupon()"
...
<script>
    ...
    import {getAnalysisPlayerCoupon} from "../../service/api/api";
    ...
    export default {
        ...
        created(){
            this.getAnalysisPlayerCoupon()
        },
        methods:{
            change(){
                this.getAnalysisPlayerCoupon()
            },
            getAnalysisPlayerCoupon(){
                this.loading=true
                getAnalysisPlayerCoupon(this.time[0],this.time[1],this.page,this.per).then(res=>{
                    this.loading=false
                    console.log("用户点券余额分布分析界面",res)
                    if(res.code==0){
                        this.list=res.data
