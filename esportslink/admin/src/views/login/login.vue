...
<script>
    ...
    export default {
        data(){
            return{
                account:getLocalStore('esportslink_account')!=null?getLocalStore('esportslink_account'):'',
                password:getLocalStore('esportslink_password')!=null?aesDe(getLocalStore('esportslink_password')):'',
                check:getLocalStore('esportslink_password')!=null?true:false,
            }
        },
        methods:{
            ...mapMutations(['TOKEN']),
            loginClick(){
                if(!!this.account&&!!this.password){
                    ...
                    userLogin(this.account,this.password).then(res=>{
                        ...
                        clearTimeout(timer)
                        if(res.code==0){
                            setLocalStore('esportslink_account',this.account)
                            if(this.check){setLocalStore('esportslink_password',aesEn(this.password))} else{
                                removeLocalStore('esportslink_password')
                            }
                            this.TOKEN(res.data.password)
