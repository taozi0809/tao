...
<script>
    ...
    export default {
        data(){
            return{
                account:getLocalStore('mahjongActive_account')!=null?getLocalStore('mahjongActive_account'):'',
                password:getLocalStore('mahjongActive_password')!=null?aesDe(getLocalStore('mahjongActive_password')):'',
                check:getLocalStore('mahjongActive_password')!=null?true:false,
            }
        },
        methods:{
            ...mapMutations(['TOKEN']),
            loginClick(){
                if(!!this.account&&!!this.password){
                    ...
                    setLocalStore('mahjongActive_account',this.account)
                    login(this.account,this.password).then(res=>{
                        console.log('登录:',res)
                        loading.close();
                        clearTimeout(timer)
                        if(res.errcode==2000){
                            if(this.check){setLocalStore('mahjongActive_password',aesEn(this.password))} else{
                                removeLocalStore('mahjongActive_password')
                            }
