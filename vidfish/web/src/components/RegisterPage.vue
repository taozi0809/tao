<template>
    <div class="register-page">
        <div class="form">
            ...
            <div class="email-message">{{lang.hasOwnProperty('lang')?langCon[lang.lang].login.loginMessage:''}}</div>
    ...
    export default{
        ...
        methods:{
            ...
            handleRegister(){
                ...
                var regEmail=/^([\w\.\-]+)\@(\w+)(\.([\w^\_]+)){1,2}$/;
                if(this.accountR==''){
                    switch (this.lang.lang) {
                        case 'ZH':this.$refs.message.Message('请输入账号');break;
                        case 'EN':this.$refs.message.Message('Please enter your email or phone number');break;
                        case 'IND':this.$refs.message.Message('Silakan masukkan email atau nomor telepon Anda');break;
                        case 'PT':this.$refs.message.Message('Por favor, digite a sua conta.');break;
                        case 'ES':this.$refs.message.Message('Por favor introduzca su número de cuenta');break;
                    }
                    return
                }else if(!regEmail.test(this.accountR)){
                    switch (this.lang.lang) {
                        case 'ZH':this.$refs.message.Message('邮箱格式错误');break;
                        case 'EN':this.$refs.message.Message('Email format error');break;
                        case 'IND':this.$refs.message.Message('Salah format email');break;
                        case 'PT':this.$refs.message.Message('Formato errado da caixa de correio');break;
                        case 'ES':this.$refs.message.Message('Formato incorrecto del buzón de correo');break;
                    }
                    return
                }
                if(!this.selectActive){
                    switch (this.lang.lang) {
                        case 'ZH':this.$refs.message.Message('请勾选同意');break;
                        case 'EN':this.$refs.message.Message('Please tick yes');break;
                        case 'IND':this.$refs.message.Message('Centang centang setuju');break;
                        case 'PT':this.$refs.message.Message('Por favor, marque su consentimiento');break;
                        case 'ES':this.$refs.message.Message('Por favor, marque "sim"');break;
                    }
                    return
                }
                vidfishUserRegist(this.userName,this.accountR,this.passwordR1,null,null,null,null,'946698415000',null,null).then(res=>{
                    console.log('注册：',res)
                    if(res.code==0){
