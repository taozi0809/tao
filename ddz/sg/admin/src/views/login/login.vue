...
<script>
    ...
    export default {
        data(){
            return{
                account:getLocalStore('sg_account')!=null?getLocalStore('sg_account'):'',
                password:getLocalStore('sg_password')!=null?aesDe(getLocalStore('sg_password')):'',
                check:getLocalStore('sg_password')!=null?true:false,
            }
        },
        // created() {
        //     register('tao','123456').then(res=>{
        //         console.log(res)
        //     })
        // },
        methods:{
            ...mapMutations(['SG_TOKEN']),
            loginClick(){
                if(!!this.account&&!!this.password){
                    ...
                    setLocalStore('sg_account',this.account)
                    login(this.account,this.password).then(res=>{
                        console.log('登录:',res)
                        loading.close();
                        clearTimeout(timer)
                        if(res.errcode==2000){
                            if(this.check){setLocalStore('sg_password',aesEn(this.password))} else{
                                removeLocalStore('sg_password')
                            }
                            this.SG_TOKEN(res.data.Token)
