...
<script>
    ...
    export default {
        data(){
            return{
                account:getLocalStore('mixing_account')!=null?getLocalStore('mixing_account'):'',
                password:getLocalStore('mixing_password')!=null?aesDe(getLocalStore('mixing_password')):'',
                check:getLocalStore('mixing_password')!=null?true:false,
            }
        },
        created() {
            this.GAMENAME('0')
            // register('骆名高','123456').then(res=>{
            //     console.log(res)
            // })
        },
        methods:{
            ...mapMutations(['MIXING_TOKEN','GAMENAME']),
            loginClick(){
                if(!!this.account&&!!this.password){
                    ...
                    setLocalStore('mixing_account',this.account)
                    login(this.account,this.password).then(res=>{
                        console.log('登录:',res)
                        clearTimeout(timer)
                        if(res.errcode==2000){
                            if(this.check){setLocalStore('mixing_password',aesEn(this.password))} else{
                                removeLocalStore('mixing_password')
                            }
                            this.MIXING_TOKEN(res.data.Token)
                            loading.close();
