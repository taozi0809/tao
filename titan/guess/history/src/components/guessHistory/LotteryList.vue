            ...
            <div class="content">
                <ul>
                    <router-link v-for="item in data" class="banner" :to ="{name:'gameDetail',params :{id:item.matchId}}">
                        ...
                        <p class="b" :class="item.status=='2'||item.status=='4'||item.status=='6'?'winning':''">
                            ...
                            <span v-if="item.status=='3'">-{{(item.betMoney).toFixed(2)+' '+item.coinName}}</span>
                            <span v-if="item.status=='4'">赢1/2，+{{((item.betMoney*(item.odds-1)/2)+item.betMoney).toFixed(2)+' '+item.coinName}}</span>
                            <span v-if="item.status=='5'">输1/2，+{{(item.betMoney/2).toFixed(2)+' '+item.coinName}}</span>
                            <span v-if="item.status=='6'">和</span>
                        </p>
                    </router-link>
                </ul>
            </div>
...
<script>
    ...
    import {football_bet_wait_list} from "../../service/api";
        ...
        mounted:function(){
            this.$nextTick(() => {
                ...
                this.scroll.on('pullingUp', () => {
                    this.isMore?this.get_football_bet_wait_list(this.userName,this.token):''
            ...
            var asyncFun = ()=>{
                if(this.userName==''||this.userName===null||this.userName==null){
                    ...
                    this.get_football_bet_wait_list(this.userName,this.token)
                }
            }
            asyncFun()
        },
        methods:{
            get_football_bet_wait_list(userName,token){
                this.page = this.page+1
                this.$refs.loadPage.LoadPage('0','加载中',true)
                football_bet_wait_list(userName,token,this.page,this.pageSize).then(res=>{
                    console.log('待开奖',res)
                    if(res.code==='0'){
