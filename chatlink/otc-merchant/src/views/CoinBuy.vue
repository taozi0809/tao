<template>
    <div class="coinSet" id="coinSet">
       ...
       <div class="nav">
           <div class="nav-item buy active" @click="$router.push({path:'/'})">购入</div>
           <div class="nav-item sale" @click="$router.push({path:'/CoinSale'})">出售</div>
       </div>
        <div class="nav-nav">
        ...
        <div class="swiper-CoinBuy">
            <div class="swiper-wrapper">
                <div class="swiper-slide">
                    <div class="coinBuy-wrapper0" ref="coinBuyWrapper0">
                        <ul class="content">
                            <van-list v-model="loadingAll" :finished="finishedAll" finished-text="没有更多了" @load="onLoad('0')">
                                <li class="item" v-for="item in listAll" :key="item.tradeId">
                                    ...
                                    <div class="col5">
                                        ...
                                        <div class="change" @click="$router.push({path:'/EditorBuy',query:{details:JSON.stringify(item)}})">修改</div>
                                    </div>
                                </li>
                            </van-list>
                        </ul>
                ...
                <div class="swiper-slide">
                    <div class="coinBuy-wrapper1" ref="coinBuyWrapper1">
                        <ul class="content">
                            <van-list v-model="loading1" :finished="finished1" finished-text="没有更多了" @load="onLoad('1')">
                                <li class="item" v-for="(item,index) in list1" :key="item.tradeId">
                                    <div class="col1">
                                        <div class="l">
                                            <span class="col1-img"><img :src="item.coinIcon" alt=""></span>
                                            <p class="col1-p">{{item.coinSymbol}}</p>
                ...
                <div class="swiper-slide">
                    <div class="coinBuy-wrapper2" ref="coinBuyWrapper2">
                        <ul class="content">
...
<script>
    ...
    export default {
        ...
        mounted:function(){
            var that = this
            this.swiper = new Swiper ('.swiper-CoinBuy', {
                on:{
        ...
        methods:{
            ...mapMutations(['IOSUPLOADURL']),
            // selectPhoto(){},
            // getPhotoUrl(url){this.IOSUPLOADURL(url)},
            tradeList(status,userName,token,pageSize){
                if(status==0){
                    this.pageAll++
                    tradeList(userName,token,status,1,this.pageAll,pageSize).then(res=>{
                        ...
                        if(res.code==0){
                            ...
                            Toast.fail({message:res.message,duration:1500})
                        }
                    })
                }
                if(status==1){
                    this.page1++
                    tradeList(userName,token,status,1,this.page1,pageSize).then(res=>{
                ...
                if(status==2){
                    this.page2++
                    tradeList(userName,token,status,1,this.page2,pageSize).then(res=>{
            ...
            updateTradeStatus(userName,token,tradeId,status){
                updateTradeStatus(userName,token,tradeId,status).then(res=>{
                    ...
                    if(res.code==0){
                        tradeList(this.userName,this.token,0,1,1,this.pageSize).then(res=>{
                            console.log('币商创建交易列表0',res)
                            if(res.code==0){
                                this.listAll=res.data.otcTrades
                            }
                        })
                        tradeList(this.userName,this.token,1,1,1,this.pageSize).then(res=>{
                            console.log('币商创建交易列表1',res)
                            if(res.code==0){
                                this.list1=res.data.otcTrades
                            }
                        })
                        tradeList(this.userName,this.token,2,1,1,this.pageSize).then(res=>{
                            console.log('币商创建交易列表2',res)
                            if(res.code==0){
...
<style lang="scss" scoped>
    .coinSet{
        ...
        .swiper-CoinBuy{
            ...
            /*border: 1px solid red;*/
            overflow: hidden;
            .coinBuy-wrapper0,.coinBuy-wrapper1,.coinBuy-wrapper2{
                position: absolute;
                top:0;
                bottom: 0;
                /*overflow: hidden;*/
